...
class ConfigManager {
  constructor(configPath = null) {
    // If configPath is the default value, use findConfigFile instead
    if (configPath === 'n8n-config.json') {
  ...
  findConfigFile() {
    ...
    for (let i = 0; i < maxLevels; i++) {
      // Look for config in n8n/ subdirectory first
      const n8nConfigPath = path.join(currentDir, 'n8n', 'n8n-config.json');
      if (fs.existsSync(n8nConfigPath)) {
        return n8nConfigPath;
      }
  ...
  loadConfig() {
    ...
    try {
      ...
      console.warn(`⚠️  Failed to load config from ${path.basename(this.configPath)}: ${error.message}`);
      return this.getDefaultConfig();
    }
  }
